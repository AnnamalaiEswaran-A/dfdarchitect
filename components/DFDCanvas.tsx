...
const DFDCanvas: React.FC<DFDCanvasProps> = ({ structure, onSelectElement, isEditMode, onUpdateElementPosition }) => {
  ...
  useEffect(() => {
    ...
    // Using .join to handle reactive updates correctly
    ...
    const nodeSelection = nodeGroup.selectAll('g.node-container')
    ...
    nodeSelection.each(function(d: any) {
      ...
      el.selectAll('*').remove(); // Clear previous contents for reactivity
